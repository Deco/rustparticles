...
fn main() {
    ...
    let physical = vulkano::instance::PhysicalDevice::enumerate(&instance).next().expect("no device!");
    ...
    let vertex_buffer = {
        ...
        impl_vertex!(Vertex, position, velocity);

        let mut arr = vec![ Vertex { position: [0.0, 0.0], velocity: [0.0, 0.0] }; 10000 ];
        for (_, v) in arr.iter_mut().enumerate() {
            // v.position = [0.0, 0.0];
            v.position = [1.0-2.0*rand::random::<f32>(), 1.0-2.0*rand::random::<f32>()];
            v.velocity = [1.0-2.0*rand::random::<f32>(), 1.0-2.0*rand::random::<f32>()];
        }

        CpuAccessibleBuffer::from_iter(&device, &BufferUsage::all(), Some(queue.family()), arr.iter().cloned()).expect("failed to create buffer")
    };
    ...
    mod vs { include!{concat!(env!("OUT_DIR"), "/shaders/src/shaders/triangle_vs.glsl")} }
    let vs = vs::Shader::load(&device).expect("failed to create shader module");
    mod fs { include!{concat!(env!("OUT_DIR"), "/shaders/src/shaders/triangle_fs.glsl")} }
    ...
    window.window().show();
    let mut t = time_milliseconds();
    let mut avsum : f64 = 0.0;
    let mut avcount = 0;

    let mut prof = Profiling::new();

    loop {
        {
            prof.section("time");
            let mut content = vertex_buffer.write(std::time::Duration::new(1, 0)).unwrap();
            let mut nt = time_milliseconds();
            let mut dt = (nt - t) as f32;
            // if (dt as u64) < 1000/60 {
            //     std::thread::sleep(std::time::Duration::from_millis(1000/60 - (dt as u64)));
            //     nt = time_milliseconds();
            //     dt = (nt - t) as f32;
            // }
            // avsum += nt - t;
            // avcount += 1;
            // if avcount == 60 {
            //     println!("{:?}", 1000.0 / (avsum/(avcount as f64)));
            //     avsum = 0.0;
            //     avcount = 0;
            // }
            t = nt;
        ...
        prof.section("build command buffer");
...
struct Profiling<'a> {
    map: std::collections::HashMap<&'a str, f64>,
    time: f64,
    section: &'a str,
    count: u64,
}
impl<'a> Profiling<'a> {
    ...
    fn section(&mut self, section: &'a str) {
        let newtime = time_milliseconds();
        if self.section != "" {
            let val = self.map.entry(self.section).or_insert(0.0);
            *val += newtime - self.time;
        } else {
            self.count += 1;
            if self.count == 300 {
                for (section, sum) in self.map.drain() {
                    println!("{:20?}: {:?}", section, sum/(self.count as f64));
                }
                self.count = 0;
            }
        }
